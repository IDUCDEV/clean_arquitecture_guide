#!/usr/bin/env python3
"""
Build PDF guides for Module 23 — Spec Driven Development.
Generates two PDFs:
...
PDF_DIR = os.path.dirname(SCRIPT_DIR)  # pdf/ directory
MODULE_DIR = os.path.dirname(PDF_DIR)  # Module root

# ── Output files ──────────────────────────────────────────────────────────────
FULL_PDF = os.path.join(OUTPUT_DIR, "23-SPEC-DRIVEN-DEVELOPMENT.pdf")
SUMMARY_PDF = os.path.join(OUTPUT_DIR, "GUIA-RESUMEN-23-SPEC-DRIVEN-DEVELOPMENT.pdf")
...
def copy_to_module_root():
    """Copy generated PDFs from dist/ to module pdf/ directory."""
    print("\n[3/3] Copying PDFs to module pdf/ directory...")
    copied = 0
    ...
    for src in [FULL_PDF, SUMMARY_PDF]:
        if os.path.exists(src):
            ...
            print(f"  [✓] {os.path.basename(src)} → {PDF_DIR}/")
            copied += 1
        else:
...
def main():
    print("=" * 60)
    print("  Module 23 — Spec Driven Development (SDD)")
    print("  PDF Builder")
    print("=" * 60)
